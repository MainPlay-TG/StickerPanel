...
def get_icon(path:str):
  if not path in ICONS:
    # log('Loading icon "%s"',path)
...
class StickerButton(QPushButton):
  def __init__(self,sticker:dict,parent:QWidget):
    ...
    self.setIcon(get_icon(sticker["sticker"].path))
  def mousePressEvent(self,event:QMouseEvent):
    if event.button()==Qt.MouseButton.RightButton:
      cb=QApplication.clipboard()
      cb.setImage(get_image(self.sticker["sticker"].path))
  def mouseMoveEvent(self,event:QMouseEvent):
    img=get_image(self.sticker["sticker"].path)
    drag=QDrag(self)
    mime=QMimeData()
...
class PackButton(QPushButton):
  ...
  def __init__(self,pack_id:str,pack:dict,parent:QWidget):
    ...
    def on_click(a0):
      ...
      if pack_id==STICKER_PANEL.current:
        return
      STICKER_PANEL.current=pack_id
      print(pack_id)
      layout=QGridLayout()
...
class MainWindow(QMainWindow):
  def __init__(self):
    ...
    self.setWindowTitle(lang["window/title"]+" (loading)")
    self.show()
    load_stickers()
